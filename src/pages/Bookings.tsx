...
import { MapPin, Calendar, Car, AlertCircle, QrCode, Clock, History as HistoryIcon } from "lucide-react";
...
const Bookings = () => {
  ...
  const getStatusBadge = (status: string) => {
      switch(status) {
          case 'confirmed': return <Badge className="bg-green-500 hover:bg-green-600">Đang hoạt động</Badge>;
          case 'completed': return <Badge className="bg-blue-500 hover:bg-blue-600">Hoàn thành</Badge>;
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans">
      ...
      <main className="container mx-auto px-4 py-8 max-w-4xl">
        <div className="flex justify-between items-end mb-8">
            ...
            <Button onClick={() => navigate('/parking')} className="bg-blue-600 hover:bg-blue-700 shadow-lg shadow-blue-200">Đặt Chỗ Mới</Button>
        </div>
        ...
        <Tabs defaultValue="all" onValueChange={setFilter} className="w-full">
            ...
            <div className="space-y-4">
                {filteredBookings.map((booking: any) => (
                    <Card key={booking.id} className="bg-white border-slate-200 hover:shadow-md transition-all group overflow-hidden">
                            ...
                            <div className="flex flex-col md:flex-row justify-between gap-6">
                                <div className="space-y-4 flex-1">
                                    ...
                                    <div className="flex gap-6 text-sm">
                                        <div className="flex items-center gap-2 text-slate-600">
                                            <div className="p-2 rounded bg-slate-100"><Car className="w-4 h-4"/></div>
                                            <span className="font-bold">{booking.vehicle_number}</span>
                                        </div>
                                        <div className="flex items-center gap-2 text-slate-600">
                                            <div className="p-2 rounded bg-slate-100"><Clock className="w-4 h-4"/></div>
                                            <span className="font-bold">{format(new Date(booking.start_time), "HH:mm dd/MM", {locale: vi})}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="flex flex-col justify-between items-end border-l border-slate-100 pl-6 md:w-48">
                                    ...
                                    {booking.status === 'confirmed' && (
                                        <Button className="w-full bg-slate-900 text-white hover:bg-slate-800 font-bold mt-4">
